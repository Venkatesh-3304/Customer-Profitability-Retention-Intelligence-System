...
SELECT
    -- TOTAL REVENUE
    (SELECT SUM(net_revenue) FROM orders) AS total_revenue,

    -- TOTAL COST
    (
        SELECT SUM(cost_of_goods + shipping_cost + delivery_region_cost + payment_processing_fee + refund_amount)
        FROM orders
    ) AS total_cost,

    -- GROSS MARGIN %
    (
        SELECT ROUND(SUM(gross_margin) / SUM(net_revenue) * 100, 2)
        FROM orders
    ...
    (
        SELECT ROUND(
            SUM(net_revenue - cost_of_goods) / SUM(net_revenue) * 100,
            2
        )
        FROM orders
    ...
    -- CLV
    (SELECT AVG(total_revenue) FROM customer_unit_economics) AS avg_clv,

    -- TOTAL MARKETING SPEND
    (SELECT SUM(spend_amount) FROM marketing_spend) AS total_marketing_spend,

    -- CAC
    (
        SELECT ROUND(
            SUM(spend_amount) / SUM(new_customers),
            2
        )
        FROM marketing_spend
    ) AS cac,

    -- ✅ FIXED CHURN RATE (THIS IS THE IMPORTANT PART)
    (
        SELECT ROUND(
            COUNT(*) FILTER (WHERE churn_flag = TRUE) * 100.0 / COUNT(*),
            2
        )
...
-- 28. CHURN KPI VIEW (ADD THIS)

CREATE OR REPLACE VIEW vw_churn_kpi AS
SELECT
    COUNT(*) AS total_subscriptions,
    COUNT(*) FILTER (WHERE churn_flag = TRUE) AS churned_subscriptions,
    ROUND(
        COUNT(*) FILTER (WHERE churn_flag = TRUE) * 100.0 / COUNT(*),
        2
    ) AS churn_rate_percent
